...
static esp_io_expander_handle_t io_expander =
    NULL; // IO expander tca9554 handle
...
esp_err_t bsp_display_brightness_set(int brightness_percent) {
  ...
  if (brightness_percent < 0 || brightness_percent > 100) {
    ESP_LOGE(TAG,
             "Invalid brightness percentage. Should be between 0 and 100.");
...
static void bsp_align_area(lv_area_t *area) {
  if (area == NULL) {
    return;
  }
  area->x1 &= ~1;
  area->y1 &= ~1;
  area->x2 = area->x1 + (((area->x2 - area->x1 + 2) & ~1) - 1);
  area->y2 = area->y1 + (((area->y2 - area->y1 + 2) & ~1) - 1);
}

#if LVGL_VERSION_MAJOR >= 9
static void rounder_event_cb(lv_event_t *e) {
  bsp_align_area(lv_event_get_invalidated_area(e));
}
#else
...
esp_err_t bsp_display_new(const bsp_display_config_t *config,
  ...
  ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi(
      (esp_lcd_spi_bus_handle_t)BSP_LCD_SPI_NUM, &io_config, &io_handle));
  ...
  ESP_ERROR_CHECK(
      esp_lcd_new_panel_co5300(io_handle, &panel_config, &panel_handle));
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config,
  ...
  esp_err_t (*touch_new)(const esp_lcd_panel_io_handle_t io,
                         const esp_lcd_touch_config_t *config,
                         esp_lcd_touch_handle_t *out_touch) = NULL;
  uint16_t x_gap = 0;

  esp_lcd_panel_io_i2c_config_t cst816s_config =
  ...
  if (ESP_OK == bsp_i2c_device_probe(cst816s_config.dev_addr)) {
    ...
    esp_lcd_panel_io_i2c_config_t ft5x06_config =
...
esp_io_expander_handle_t bsp_io_expander_init(void) {
  BSP_ERROR_CHECK_RETURN_ERR(bsp_i2c_init());
  if (!io_expander) {
    BSP_ERROR_CHECK_RETURN_NULL(esp_io_expander_new_i2c_tca9554(
        i2c_handle, BSP_IO_EXPANDER_I2C_ADDRESS, &io_expander));
  }
  return io_expander;
}
...
static lv_display_t *bsp_display_lcd_init() {
  ...
  const uint32_t buffer_size = BSP_LCD_H_RES * LVGL_BUFFER_HEIGHT;
...
#if LVGL_VERSION_MAJOR >= 9
  lv_display_add_event_cb(disp, rounder_event_cb, LV_EVENT_INVALIDATE_AREA,
                          NULL);
#else
